...
1. `node ${CLAUDE_PLUGIN_ROOT}/bin/flow-tools.cjs checklist-gen --sd .spec-flow/specs/<feature>/SD.md --feature <feature>` — writes the scaffold. **If a CHECKLIST.yaml already exists the engine returns `CHECKLIST_EXISTS` and stops** — pass `--force` only to deliberately regenerate from SD (this overwrites all filled assertions; only do it when §13.2 changed substantially via `/sf:resync` and the old checklist is no longer valid).
   - **Token scaffold auto-detects auth model** (via `scripts/detect-auth.sh` from the manual-test skill): Summer/APISIX projects (or anything not deterministically classified) get the `payload:` → `X-Userinfo` form; plain-JWT projects (Spring OAuth2 resource server, Node/Python/Go/dotnet JWT libs) get `bearer: "${JWT}"` instead. Wrong guess (e.g. project is JWT but detection came back `unknown`) → every test 401s on the wrong header; fix by regenerating with an explicit `--auth jwt-basic` (or `--auth summer`), or just hand-edit the `tokens.user_token` block before filling the rest.
