 ...
 *   - Default is 'legacy' (dark-launch — FR-002, D3).
 *   - Only an explicit taskCore.engine = 'native' enables the native engine.
 *   - Any unknown value produces a stderr warning and falls back to 'legacy'
 *     (FR-003 — no silent implicit native).
 *
 * Error handling:
 *   - ENOENT (missing file) → return 'legacy' (dark-launch safe default).
...
function readEngineConfig(_inject) {
  ...
  // Missing file → 'legacy' (dark-launch default, FR-002).
  if (config === null) {
    return 'legacy';
  }

  // Extract engine value; absent / null / '' → 'legacy'.
  ...
  // Unknown value — warn to stderr and fall back to legacy (FR-003).
  // Must not silently enable native for an unrecognised value.
  process.stderr.write(
    `[engine-selector] Unknown taskCore.engine '${engine}', defaulting to legacy\n`
  );
  return 'legacy';
}
