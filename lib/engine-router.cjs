 ...
 *   - The native CRUD core (task-core.cjs + tag-manager.cjs) — the default, or
 *   - The AI hybrid stub (ai-hybrid.cjs, lazily required), or
 *   - Legacy mode (fail-open envelope) only when engine is explicitly 'legacy'
 *     (rollback escape hatch — see scripts/rollback.cjs).
 *
 ...
 *   3. Extract taskCore.engine; missing config or missing engine → native (default).
 *   4. engine='legacy' → legacy envelope (fail-open rollback escape hatch, FR-017).
 *   5. engine='native' (or unset/unknown) → classify op as CRUD or AI:
...
async function routeToEngine(operation, args) {
  ...
  // 3. Extract engine value; absent config or absent engine → native (default).
  const rawEngine = config && config.taskCore && config.taskCore.engine;
  const engine = rawEngine || 'native';

  // 4. Explicit engine='legacy' → legacy envelope (rollback escape hatch, FR-017).
  if (engine === 'legacy') {
    return LEGACY_ENVELOPE;
  }

  // 5. Anything else — 'native', the default, or an unrecognised value — dispatches
  // to the native engine (no silent fallback to legacy, a removed dependency).
  if (engine !== 'native') {
    console.warn(
      `[engine-router] unknown engine value '${engine}', defaulting to native`
    );
  }
